"""
Functions for creating and managing decryption policies in PAN-OS.

This module provides functionality to:

- Import decryption rules from configuration files
- Handle domain prefixes for user identities based on device type
- Create decryption rules with appropriate parameters for both standalone firewalls and Panorama
...
def decryption_policy(panos_device, root_policy_folder, target_environment):
    """
    Create a list of decryption rules by parsing and processing rule definitions from the specified folder.

    Args:
        panos_device: The PAN-OS device object (Firewall or Panorama).
        root_policy_folder: The folder containing the decryption rule definitions.
        target_environment: The target environment for the policy (e.g., "lab" or "prod").

    Returns:
        tuple: A tuple containing two elements:
            - A list of decryption rule objects.
            - A set of deduplicated group tags used in the decryption policy.
    """
    ...
    if settings.VERBOSE_OUTPUT:
        ...
        if "PRE" in root_policy_folder:
            table_title = "Decryption Policy Pre-Rules"
        elif "POST" in root_policy_folder:
            table_title = "Decryption Policy Post-Rules"
        else:
            table_title = "Decryption Policy Rules"
    ...
    # This is the end of the decryption rule base
